...
std::shared_ptr<ImageData> loadImageDataSync(int image_id) {
    ...
    try {
        ...
        // Load depth - try compressed format first, then fall back to original
        std::string depth_suffix = geom_consistency ? "/depths_geom" : "/depths";
        bool depth_loaded = false;
        
        // Try .cdmb (compressed) first
        ret = snprintf(buf, sizeof(buf), "%s/ACMMP/2333_%08d%s.cdmb", 
                      dense_folder.c_str(), image_id, depth_suffix.c_str());
        if (ret >= 0 && ret < static_cast<int>(sizeof(buf))) {
            if (CompressedDMB::readDepthCompressed(std::string(buf), data->depth) == 0) {
                depth_loaded = true;
            }
        }
        
        // Fall back to .dmb (original format)
        if (!depth_loaded) {
        ...
        // Load normal - try compressed format first, then fall back to original
        bool normal_loaded = false;
        
        // Try .cdmb (compressed) first
        ret = snprintf(buf, sizeof(buf), "%s/ACMMP/2333_%08d/normals.cdmb", 
                      dense_folder.c_str(), image_id);
        if (ret >= 0 && ret < static_cast<int>(sizeof(buf))) {
            if (CompressedDMB::readNormalCompressed(std::string(buf), data->normal) == 0) {
                normal_loaded = true;
            }
        }
